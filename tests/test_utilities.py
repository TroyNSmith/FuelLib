...
class TestUtilityFunctions(unittest.TestCase):
    ...
    def test_epsilon_to_characteristic_temperature(self):
        ...
        result = fl.convert.epsilon_to_characteristic_temperature(Q_(1000.0, "J/mol"))
        expected = (Q_(1000.0, "J/mol") / fl.constants.N_A) / fl.constants.k_B
        self.assertAlmostEqual(result.magnitude, expected.to("K").magnitude, places=10)

    def test_epsilon_to_characteristic_temperature_zero(self):
        """Test epsilon conversion with zero input."""
        result = fl.convert.epsilon_to_characteristic_temperature(Q_(0.0, "J/mol"))
    ...
    def test_C2K(self):
        ...
        self.assertAlmostEqual(
            fl.convert.C2K(Q_(0.0, "degC")).magnitude, 273.15, places=10
        )
        # 25°C = 298.15 K
        self.assertAlmostEqual(
            fl.convert.C2K(Q_(25.0, "degC")).magnitude, 298.15, places=10
        )
        # 100°C = 373.15 K
        self.assertAlmostEqual(
            fl.convert.C2K(Q_(100.0, "degC")).magnitude, 373.15, places=10
        )
    ...
    def test_K2C(self):
        ...
        # 273.15 K = 0°C
        self.assertAlmostEqual(
            fl.convert.K2C(Q_(273.15, "K")).magnitude, 0.0, places=10
        )
        # 298.15 K = 25°C
        self.assertAlmostEqual(
            fl.convert.K2C(Q_(298.15, "K")).magnitude, 25.0, places=10
        )
        # 373.15 K = 100°C
        self.assertAlmostEqual(
            fl.convert.K2C(Q_(373.15, "K")).magnitude, 100.0, places=10
        )

    def test_C2K_K2C_roundtrip(self):
        """Test roundtrip conversion between Celsius and Kelvin."""
        original_c = 42.5
        kelvin = fl.convert.C2K(Q_(original_c, "degC"))
        result_c = fl.convert.K2C(kelvin)
        self.assertAlmostEqual(result_c.magnitude, original_c, places=10)
