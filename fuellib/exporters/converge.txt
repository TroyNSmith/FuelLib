...
def export_converge(
    ...
    def calculate_mixture_properties(T_array, fuel):
        ...
        rho = fuel.mixture_density(T_q).ustrip("kg/m^3")
        mu = fuel.mixture_dynamic_viscosity(T_q).ustrip("Pa*s")
        pv = fuel.mixture_vapor_pressure(T_q).ustrip("Pa")
        surface_tension = fuel.mixture_surface_tension(T_q).ustrip("N/m")
        thermal_conductivity = fuel.mixture_thermal_conductivity(T_q).ustrip("W/(m*K)")
    ...
    def calculate_component_properties(T_array, fuel, comp_idx):
        ...
        # Methods return all compounds; select comp_idx's column since comp_idx is no longer accepted.
        T_q = u.Q(T_array, "K")
        rho = fuel.density(T_q)[:, comp_idx].ustrip("kg/m^3")
        mu = fuel.viscosity_dynamic(T_q)[:, comp_idx].ustrip("Pa*s")
        pv = fuel.psat(T_q)[:, comp_idx].ustrip("Pa")
        surface_tension = fuel.surface_tension(T_q)[:, comp_idx].ustrip("N/m")
        thermal_conductivity = fuel.thermal_conductivity(T_q)[:, comp_idx].ustrip(
            "W/(m*K)"
        )
        Lv = fuel.latent_heat_vaporization(T_q)[:, comp_idx].ustrip("J/kg")
        Cl = fuel.Cl(T_q)[:, comp_idx].ustrip("J/(kg*K)")
