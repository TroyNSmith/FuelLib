...
def _family_codes(fuel):
    """Map hydrocarbon_types labels to the legacy 0-3 Pele family codes."""
    codes = {"aromatic": 1, "cyclic": 2, "alkene": 3}
    return np.array([codes.get(t, 0) for t in fuel.hydrocarbon_types])
    ...
    return pd.DataFrame(
        {
            ...
            "Family": [st.mode(_family_codes(fuel)).mode],
            "Y_0": [1.0],
            "MW": [fuel.mean_molecular_weight().ustrip(converter.MW)],
...
def export_pele(
    ...
    if liq_prop_model.lower() == "gcm":
        ...
        ref_T = u.Q(298.15, "K")
        if export_mix:
            rho = fuel.mixture_density(ref_T)
        else:
        ...
        if psat_antoine:
            if export_mix:
                # No more auto per-mixture range, so pick 273.15 K to the mixing-rule Tb
                Tb_mix = float(
                    fl.utility.mixing_rule(fuel.Tb.ustrip("K"), fuel.Y2X(fuel.Y_0))
                )
                T_fit = u.Q(np.linspace(273.15, Tb_mix, 20), "K")
                (
                    psat_A,
                    psat_B,
                    psat_C,
                    psat_D,
                ) = fuel.mixture_vapor_pressure_antoine_coeffs(T_fit, unit=converter.P)
            else:
                # No more auto per-compound range, so pick 273.15 K to the lowest Tb
                Tb_min = float(min(fuel.Tb.ustrip("K")))
                T_fit = u.Q(np.linspace(273.15, Tb_min, 20), "K")
                psat_A, psat_B, psat_C, psat_D = fuel.psat_antoine_coeffs(
                    T_fit, unit=converter.P
                )

            df["psat_A"] = psat_A
            df["psat_B"] = psat_B
            df["psat_C"] = psat_C
            df["psat_D"] = psat_D.ustrip(converter.P)
